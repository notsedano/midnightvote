-- Set up site_settings table with proper permissions
-- This complete script fixes banner image display issues for all users

-- First, check if the table exists
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'site_settings') THEN
    -- Create the table if it doesn't exist
    CREATE TABLE public.site_settings (
      id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
      key TEXT NOT NULL UNIQUE,
      value TEXT,
      created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
      updated_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL
    );
    
    -- Add comment
    COMMENT ON TABLE public.site_settings IS 'Stores site-wide settings like banner URLs';
  END IF;
END
$$;

-- Make sure RLS is enabled
ALTER TABLE public.site_settings ENABLE ROW LEVEL SECURITY;

-- Drop existing policies to avoid conflicts
DROP POLICY IF EXISTS "Anyone can read settings" ON public.site_settings;
DROP POLICY IF EXISTS "Authenticated users can manage settings" ON public.site_settings;
DROP POLICY IF EXISTS "Anon users can manage settings temporarily" ON public.site_settings;

-- Create policies - anonymous users need to be able to read the settings
CREATE POLICY "Anyone can read settings" 
ON public.site_settings 
FOR SELECT 
USING (true);

-- Only authenticated users can modify settings
CREATE POLICY "Authenticated users can manage settings" 
ON public.site_settings 
FOR ALL 
USING (auth.role() = 'authenticated');

-- Create function to update site_settings that bypasses RLS
-- This allows the function caller to update settings regardless of RLS
CREATE OR REPLACE FUNCTION public.update_site_setting(setting_key TEXT, setting_value TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER -- This runs with the privileges of the function creator
AS $$
BEGIN
  -- Insert or update the setting
  INSERT INTO public.site_settings (key, value)
  VALUES (setting_key, setting_value)
  ON CONFLICT (key) 
  DO UPDATE SET 
    value = setting_value,
    updated_at = now();
    
  RETURN TRUE;
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error updating site setting: %', SQLERRM;
    RETURN FALSE;
END;
$$;

-- Create a function to get banner URLs that bypasses RLS
-- This ensures anonymous users can get banner URLs
CREATE OR REPLACE FUNCTION public.get_banner_urls()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER -- This runs with the privileges of the function creator
AS $$
DECLARE
  result JSONB;
BEGIN
  SELECT jsonb_build_object(
    'banner1', COALESCE((SELECT value FROM public.site_settings WHERE key = 'login_banner1'), ''),
    'banner2', COALESCE((SELECT value FROM public.site_settings WHERE key = 'login_banner2'), '')
  ) INTO result;
  
  RETURN result;
EXCEPTION
  WHEN OTHERS THEN
    RETURN jsonb_build_object('error', SQLERRM);
END;
$$;

-- Add initial settings if they don't exist
INSERT INTO public.site_settings (key, value)
VALUES 
  ('login_banner1', ''),
  ('login_banner2', '')
ON CONFLICT (key) DO NOTHING;

-- Show current settings for verification
SELECT * FROM public.site_settings; 